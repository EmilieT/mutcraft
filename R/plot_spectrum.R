...
mc.plotSpectrum <-function(mut.sum, yaxis = c("count", "prop")[1],
    ...
    if (yaxis == "count") {
      p <-ggplot(mut.sum, aes(
          ...
          y = count,
          fill = mut
        )) + geom_bar(stat = "identity")  + theme_bw() + xlab("Mutation Type") + ylab("Count") + ggtitle("Mutation Spectrum (count)")
      if (print.num)
      ...
      p <- ggplot(mut.sum, aes(
          ...
          y = prop,
          fill = mut
        )) + geom_bar(stat = "identity") + scale_y_continuous(labels = percent_format()) + theme_bw() + xlab("Mutation Type") + ylab("Percentage") + ggtitle("Mutation Spectrum (%)")
      if (print.num)
